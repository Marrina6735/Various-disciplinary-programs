...
begin
  ...
  case CopyForm.RadioGroup1.itemindex of
    0:
      begin
        xycopytoclipboard;
        showmessage('Graph copied to clipboard');
      end;
    1:
      begin
        xyprint(true);
    ...
    3:
      begin
        xysaveasbitmap(xybmpname, msgon, ok);
      end;
    4:
      begin
        if not cvmode then
          xysaveasmetafile(xywmfname, msgon, ok, 1)
        else
          xysaveasmetafile(xywmfname, msgon, ok, 1, xycvplot);
      end;
    5:
      begin
        if not cvmode then
          xysaveasmetafile(xywmfname, msgon, ok, 2)
        else
          xysaveasmetafile(xywmfname, msgon, ok, 2, xycvplot);
      end;
    6:
      begin
        if not cvmode then
          xysaveasmetafile(xywmfname, msgon, ok, 4)
        else
          xysaveasmetafile(xywmfname, msgon, ok, 4, xycvplot);
      end;
  end;
end;

function SaveFileNameDlg(var fName: string; aFilterIndex: integer): boolean;
var
...
begin
  ...
  SaveDialog.Filter := 'Все файлы (*.*)|*.*|RTF (*.rtf)|*.rtf|Текст (*.txt)|*.txt'+
                        '|Html (*.htm)|*.htm|bmp (*.bmp)|*.bmp|wmf (*.wmf)|*.wmf';

    if (aFilterIndex < 1) or (aFilterIndex > 6) then aFilterIndex := 1;
  SaveDialog.FilterIndex := aFilterIndex;
  GetDir(0, OldDir);
  ChDir(ExtractFilePath(Application.ExeName));
  SaveDialog.FileName := ExtractFileName(Application.ExeName);
...
var
  ...
  procedure getname1;
  begin
   xybmpname := ExtractFileDir(Application.ExeName);

    SaveFileNameDlg(xybmpname, 5);
    s := xybmpname;
  ...
  procedure getname2;
  begin
    xywmfname := ExtractFileDir(Application.ExeName);
    SaveFileNameDlg(xywmfname, 6);
    s := xywmfname;
  ...
  with ComboBox1 do
    with items do
      ...
      begin
        ...
        for i := 1 to n - 1 do
          add(tekst(i));
        if (oldindex > n - 2) then
          oldindex := 0;
        itemindex := oldindex;
        enabled := true;
        buf := true;
        bufl := xybuffers[1].lbl;
        setmode(1);
      end;
end;
