...
uses winapi.Windows , System.UITypes, System.Types,
   System.SysUtils, System.Classes, Vcl.Graphics,
   Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls, Vcl.Clipbrd, xygraph;
...
procedure setauton(n:integer;v:double;a:integer);
procedure setn(n:integer;v:double);
procedure resetn(n:integer);
procedure resetall;
procedure autoreset(code:integer);
function getn(n:integer):double;
function prm(n:integer):double;
...
procedure position0(x,y,z:double;var xp,yp:integer);
...
procedure simpletext(cl:Tcolor;txt:shortstring;xp,yp,xjust,yjust:integer);
procedure bigtext(cl:Tcolor;txt:shortstring;xp0,yp0,xjust,yjust,mode,angle,code:integer);
procedure labelangle(cl:Tcolor;txt:shortstring;mode,bi,xp,yp,ang:integer;var dl:integer);

procedure initwmf;
procedure openwmf(var s:shortstring;var ok:boolean);
...
procedure textwmfout(x,y:integer;s:shortstring);
procedure bitmapwmf(x1,y1,x2,y2:integer);

procedure inittext3d(f,x1,x2,y1,y2:double;var ok:boolean);
procedure exittext3d; 
procedure text3d(s:shortstring;x,y,z:double;m1:integer;lbl:boolean;
       oct,ml:integer;zlvert:boolean;f:double);
...
var xycanvas : Tcanvas;
    ...
    transval : double;                {transparancy level}
    ...
    scale3d : double;                 {overall schaal factor}

type scaletype = record               {vaste schaal instellingen}
               min,max,fm,fac : double;
               nc,nf,ndec : integer;
               fix,go : boolean;
               exp : shortstring;
            end;
...
type graftype3d = record                       {3d uitbreidingen van buf}
                  ...
                  z1,z2,c1,c2 : double;        {hoogtes}
                  xf0,xf1,yf0,yf1 : double;    {zoom factoren}
                  s1,s2,s3,s4,s5,s6 : double;  {surface waarden}
                  hm,drm : integer;            {hoogtemode, driehmode}
                  f3dsize : integer;           {font size ivm 3d text}
                  empt : double;               {leeg veld}
                  hasempt : boolean;           {er is een leegveld}
                  end;
     grafstype3d = array[1..maxgraf] of graftype3d;

var graphs3d : grafstype3d;
    xmid,ymid,zmid : double;      {middens van assen}
    facx,facy,facz : double;      {schaal factoren voor assen}
    ifacx,ifacy,ifacz : double;   {inverse factoren}
    sinp,cosp,sina,cosa : double; {sin en cos van kijkhoek}
    isinp,icosp,isina,icosa : double; {inverse factoren}
    sinr,cosr : double;           {sin en cos van draaiing}
    fac3d, r3d : double;          {reken factoren}
    ifac3d, ir3d : double;        {inverse factoren}
    xpc, xpc0, ypc : integer;     {quasi midden vam beeldscherm}
    xc0, yc0 : integer;           {echte midden van beeldscherm}
    pixx3d, pixy3d, pixz3d : double; {aantal pixels voor lengte 1}
    kijkhoek,kijkhoogte : integer;{-id-}
    viewx,viewy,viewz : double;   {kijkpunt}
    empty : double;               {lege waarde bij surface}
...
type fonttype = record name: shortstring; size:integer; style:Tfontstyles; end;
...
var setval : array of double;     {actuele waarden}
...
type setting = record             {instelling van obscure parameters}
               nr : integer;      {codenr}
               def : double;      {act waarde, default waarde}
               min,max : double;  {min en max bereik}
               isint : boolean;   {integer waarde of niet}
               txt:shortstring;        {beschrijving}
               end;
...
function getn(n:integer):double; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
begin
...
procedure setn(n:integer;v:double); {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
 {set een waarde}
var t : integer;
begin
  if (n=0) then
...
procedure setauton(n:integer;v:double;a:integer); {XXXXXXXXXXXXXXXXXXXXXXXXXXXX}
var t : integer;
begin
...
function prm(n:integer):double; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
...
procedure simpletext(cl:Tcolor;txt:shortstring;xp,yp,xjust,yjust:integer); {XXXXXXXX}
begin
  if (txt='') then exit;
...
var lines : array of shortstring;
...
procedure position0(x,y,z:double;var xp,yp:integer); {XXXXXXXXXXXXXXXXXXXXXXXXX}
var x1,x2,y1,y2,ff,t,f : double;
begin
...
procedure positionr(x,y,z:double;var xs,ys:double); {XXXXXXXXXXXXXXXXXXXXXXXXX}
var x1,x2,y1,y2,ff,t : double;
begin
  x := (x-xmid)*facx; y := (y-ymid)*facy; z := (z-zmid)*facz;
  x1 := x*cosp - y*sinp; y1 := x*sinp + y*cosp; {z1 := z}    {draaiing om z-as}
  x2 := x1; y2 := y1*sina + z*cosa; {z2 = y1*cosh - z1*sinh} {draaiing om as in xy vlak}
...
procedure inittext3d(f,x1,x2,y1,y2:double;var ok:boolean); {XXXXXXXXXXXXXXXXXXX}
...
procedure showtext3d(s:shortstring;col:Tcolor;x,y,z:double; {XXXXXXXXXXXXXXXXXXXXXXX}
...
var tw,th,tw0,th0,xr,yr,xp,yp,xt,yt,c1,c2,i,j,t : integer;
    u0,v0,du,dv,uw,vw,u,v : double;
    ipx,ipy,ipz,d,xs,ys : double;
    ipu,ipv,fu,fv,dipu,dipv : double;
    rot : boolean;
...
procedure position(u,v:double;var xr,yr: integer); {---------------------------}
begin
...
procedure text3d(s:shortstring;x,y,z:double;m1:integer;lbl:boolean; {XXXXXXXXXXXXXXX}
       oct,ml:integer;zlvert:boolean;f:double);
  {m1 :  1-4  = X-as, 5-8  = Y-as, 9-12 = Z-as, 13-16 = hoeken}
var x2,y2,z2,d : double;
    kp,av,cl : boolean;
    vl,ri,ju : integer;
...
(*
Procedure CanvasTextOutAngle(C:TCanvas; X,Y:Integer; Angle:Word; S:shortstring);
Var
  LogRec: TLOGFONT;
  OldFontHandle,
  NewFontHandle: HFONT;
...
begin
  domove := (buf[3]=$0214);
  if (domove and lastmove) then {overschrijf vorige _ перезаписать предыдущую}
    begin
      Wint := filepos(fn)-integer(bufd[1]);
      seek(fn,Wint);
...
procedure fillintext(s:shortstring; pos:integer; nul:boolean); {WWWWWWWWWWWWWWWWWWWW}
var i : integer;
begin
  if (s='') then s := ' ';
  if nul then s := s + #0;
...
procedure startwmf(var s:shortstring;var ok : boolean); {WWWWWWWWWWWWWWWWWWWWWWWWWWW}
var i : integer;
begin
...
procedure bitmap(name:shortstring; xp,yp:integer; fac:double); {WWWWWWWWWWWWWWWWWWWW}
...
procedure textout(x,y:integer;txt:shortstring); {WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW}
begin
  bufd[1] := 0; buf[3] := $0521; buf[4] := length(txt);
  fillintext(txt,5,false);
  inc(bufd[1],2);
...
procedure comment(txt:shortstring); {WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW}
begin
...
procedure openwmf(var s:shortstring;var ok:boolean); {WWWWWWWWWWWWWWWWWWWWWWWWWWWWWW}
begin
...
procedure textwmfout(x,y:integer;s:shortstring); begin textout(x,y,s); end;
